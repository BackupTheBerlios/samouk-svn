...
/// Everything below uses "section" terminology - each "section" is a topic.
    ...
    while ($section <= $course->numsections) {
            ...
            // prepare data to show in section
        ...
        if (!empty($displaysection) and $displaysection != $section) {
            // this section is not in a list of sections to display
            if ($showsection) {
        ...
        if ($showsection) {
            ...
            if (!$thissection->visible) {
                // for teachers, who can see hidden sections
                $sectionstyle = ' hidden';
            ...
            if (!has_capability('moodle/course:viewhiddensections', $context) and !$thissection->visible) {   // Hidden for students
                ...
                echo get_string('chapter','samouk').' '.$section;
                echo '  </div>';
	            ...
	            if (isediting($course->id) && has_capability('moodle/course:update', get_context_instance(CONTEXT_COURSE, $course->id))) {
	                ...
	                if ($course->marker == $section) {  // Show the "light globe" on/off
	                    echo '<a href="view.php?id='.$course->id.'&amp;marker=0&amp;sesskey='.$USER->sesskey.'#section-'.$section.'" title="'.$strmarkedthistopic.'">'.
	                         '<img src="'.$CFG->pixpath.'/i/marked.gif" alt="'.$strmarkedthistopic.'" /></a>&nbsp;';
	                } else {
	                    echo '<a href="view.php?id='.$course->id.'&amp;marker='.$section.'&amp;sesskey='.$USER->sesskey.'#section-'.$section.'" title="'.$strmarkthistopic.'">'.
	                         '<img src="'.$CFG->pixpath.'/i/marker.gif" alt="'.$strmarkthistopic.'" /></a>&nbsp;';
	                }
            ...
            if ($USER->su_isadvanced) {
	            ...
	            if ($displaysection == $section) {      // Show the zoom out boxes
	                echo '<a href="view.php?id='.$course->id.'&amp;topic=0#section-'.$section.'" title="'.$strshowalltopics.'">'.
	                     '<img src="'.$CFG->pixpath.'/i/all.gif" alt="'.$strshowalltopics.'" /></a><br />';
	            } else {
	                // display icon to zoom in this box
