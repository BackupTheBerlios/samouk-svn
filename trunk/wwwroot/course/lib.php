...
function print_section_add_menus($course, $section, $modnames, $vertical=false, $return=false) {
    ...
    if ($resources === false) { 
        ...
        foreach($modnames as $modname=>$modnamestr) {
            ...
            // assign activities to different popups....
            if (function_exists($gettypesfunc)) {
                ...
                foreach($types as $type) {
                    ...
                    } else if (in_array($type->type, $samouk_hw)) {
                        $homeworks[$type->type] = $type->typestr;
                    }
                }
            } else {
                // ...or by name 
                if (in_array($modname, $samouk_res)) {
                    $resources[$modname] = $modnamestr;
                } else if (in_array($modname, $samouk_act)) {
                    $activities[$modname] = $modnamestr;
                } else if (in_array($modname, $samouk_hw)) {
                    $homeworks[$modname] = $modnamestr;
                }
            }
        }
    }
    ...
    $straddhomework = get_string('modulenameplural','assignment');
    ...
    if (!empty($activities) && $USER->su_isadvanced) {
        $output .= ' ';
        $output .= popup_form("$CFG->wwwroot/course/mod.php?id=$course->id&amp;section=$section&amp;sesskey=".sesskey()."&amp;add=",
                    $activities, "section$section", "", $straddactivity, 'mods', $straddactivity, true);
    }
    
    if (!empty($homeworks) && $USER->su_isadvanced) {
        $output .= ' ';
        $output .= popup_form("$CFG->wwwroot/course/mod.php?id=$course->id&amp;section=$section&amp;sesskey=".sesskey()."&amp;add=",
                    $homeworks, "hwsection$section", "", $straddhomework, 'mods', $straddhomework, true);
    }
...
function print_course($course) {
    ...
    /// first find all roles that are supposed to be displayed (teachers)
    $teacherLogged = false; $managersnames = "";
//    if ($managerroles = get_config('', 'coursemanager')) {
//        $coursemanagerroles = split(',', $managerroles);
//        foreach ($coursemanagerroles as $roleid) {
//            $role = get_record('role','id',$roleid);
//            if ($users = get_role_users($roleid, $context, true, '', 'u.lastname ASC', true)) {
//            	// remember all teachers to add menu for them 
//            	if ($role->name == "Teacher") {
//            	   $teachers = $users;
//            	}
//                foreach ($users as $teacher) {
//                	// try if current logged user is a teacher of the course
//                	if ($teacher->id == $USER->id) {
//                		$teacherLogged = true;
//                	}
//                    $fullname = fullname($teacher, has_capability('moodle/site:viewfullnames', $context)); 
//                    $namesarray[] = role_get_name($role, $context).
...
//        
//        if (!empty($namesarray)) {
//        	$managersnames = "<ul class=\"teachers\">\n<li>".implode("</li>\n<li>", $namesarray)."</li></ul>";
//        }
//    }
    
    if ($managerroles = get_config('', 'coursemanager')) {
        // read all roles pointed as coursemanagers in configuration (default is editingteachers)
        $coursemanagerroles = split(',', $managerroles);
        foreach ($coursemanagerroles as $roleid) {
            $role = get_record('role','id',$roleid);
            // go throught all managers (teachers) in given role
            if ($users = get_context_users_byrole($context, $roleid, array('id','firstname','lastname'), NULL, 'u.lastname ASC')) {
                // remember all teachers to add menu for them 
                if ($role->name == "Teacher") {
                   $teachers = $users;
                }
    ...
    // invisible courses is visible only for its creators
    if (!$course->visible && !$teacherLogged && !has_capability('moodle/course:viewhidden',$course->context))
        return;
    
    $linkcss = $course->visible ? '' : ' class="dimmed" ';

    // kowy - rewriten code generating course info 
    echo '<div class="coursebox">';
    // TODO create graphics
    echo '<div class="graphics"></div>';
    
    // add course control if user is the owner of the course
    if ($teacherLogged) {
    	$options=array();
    	...
    	echo "<br/>\n";
    	print_single_button($CFG->wwwroot.'/course/delete.php', $options, get_string('delete'));
    	echo "<br/>\n";
    	echo '</div>';
    }
    
    echo '<div class="info'.($teacherLogged?' thininfo':'').'">';
    // print a creation date
    echo '<div class="left">'.
            get_string('course.creationdate', 'samouk').userdate($course->timecreated, get_string('strftimedate'))
            .'</div>'; 
    ...
    //echo get_string('category').': '.$category->name;
    //echo '</div>';      // left
    
    // add information about enrolnment length
    echo '<div class="left">';
    print get_string("enrolperiod").': ';
    $enroldays = getdate($course->enrolperiod);
    print get_string('nummonths','', $enroldays['mon']);
    echo '</div>';
    
    // add cost information
    echo '<div class="right"';
    if ($course->cost != 0) {
        echo ' title="'.get_string("requirespayment").'">'.' '.get_string("cost").': ';
        echo $course->cost."&nbsp;".$course->currency;
    } else {
        echo '>';
    	echo get_string('free', 'samouk');
    }
    echo '</div>';  // cost
    echo '</div>';  // info
    echo '</div>';  // coursebox
...
/// Over time this can include all sorts of information
    ...
    if (!empty($courses) || !empty($rcourses) || !empty($rhosts)) {
        ...
        // kowy - allways should exist link to full list, because there is a "Create new course" button
        //if (count_records("course") > (count($courses) + 1) ) {  // Some courses not being displayed (foreign courses exist)
...
function make_editing_buttons($mod, $absolute=false, $moveselect=true, $indent=-1, $section=-1) {
    ...
    // Generate Hide/Show section button
    $hideshow = '';
    if ($USER->su_isadvanced && has_capability('moodle/course:activityvisibility', $modcontext)) {
        if ($mod->visible) {
    ...
    if ($USER->su_isadvanced && $mod->groupmode !== false) {
    ...
    $leftright = '';
    if ($USER->su_isadvanced && has_capability('moodle/course:update', get_context_instance(CONTEXT_COURSE, $mod->course))) {
    ...
    $deleteicon = '<a class="editing_delete" title="'.$str->delete.'" href="'.$path.'/mod.php?delete='.$mod->id.'&amp;sesskey='.$sesskey.$section.'">'.
            '<img src="'.$CFG->pixpath.'/t/delete.gif" class="iconsmall" alt="'.$str->delete.'" />'.
            '</a>'."\n";
    $updateicon = '<a class="editing_update" title="'.$str->update.'" href="'.$path.'/mod.php?update='.$mod->id.'&amp;sesskey='.$sesskey.$section.'">'.
            '<img src="'.$CFG->pixpath.'/t/edit.gif" class="iconsmall" alt="'.$str->update.'" />'.
            '</a>'."\n";

    // kowy - change action command icons
